...
module.exports = {
  ...
  async execute(client, message, args) {
    ...
    const human = message.guild.members.cache.filter(
      (member) => !member.user.bot
    ).size;

    let onlineMembers = message.guild.members.cache.filter(
      (member) => member.presence?.status == "online"
    ).size;

    let dndMembers = message.guild.members.cache.filter(
      (member) => member.presence?.status == "dnd"
    ).size;

    let idelMembers = message.guild.members.cache.filter(
    ...
    let Embed = new MessageEmbed()
      .setColor("BLUE")
      .setAuthor({
        name: `${message.guild.name}'s Info`,
      ...
      .addFields(
        ...
        },
        {
          ...
          value: ` **${
            onlineMembers + dndMembers + idelMembers
          }** Online | **${bots}** Bots | **${human}** Humans  `,
          inline: true,
        },
        {
          name: `💬 Channels (${message.guild.channels.cache.size}) & Roles:`,
          value: `** ${
            channels.filter((channel) => channel.type === "GUILD_TEXT").size
          }** Text |  **${
            channels.filter((channel) => channel.type === "GUILD_VOICE").size
          } ** Voice | ** ${message.guild.roles.cache.size} **Roles `,
          inline: true,
        },
        {
          name: "⭐ Emojies & stickers:",
          value: `** ${stickers} ** Stickers | ** ${emojis} **Emojies `,
          inline: true,
        },
        {
          name: `💎 Boost Tier(${message.guild.premiumTier.replace(
            "TIER_",
            ""
          )}) state : `,
          value: `Boost Tier(** ${message.guild.premiumTier.replace(
            "TIER_",
            ""
